...
public class TransportistaRepositoryImpl implements TransportistaRepository, Serializable{
	...
	public Optional<Transportista> findByNumeroDocumento(String numeroDocumento) throws Exception {
		TypedQuery<Transportista> query = em.createQuery("SELECT t FROM Transportista t WHERE t.numeroDocumento = ?1", Transportista.class).setParameter(1, numeroDocumento);
		List<Transportista> transportistas = query.getResultList();
		if (transportistas != null && !transportistas.isEmpty())
			return Optional.of(transportistas.get(0));
	...
	public Optional<Transportista> findByNombreUsuarioYClave(String nombreUsuario, String clave) throws Exception {
		TypedQuery<Transportista> query = em.createQuery("SELECT t FROM Transportista t WHERE t.nombreUsuario = ?1 AND t.clave = ?2", Transportista.class).setParameter(1, nombreUsuario);
		query.setParameter(2, clave);
		List<Transportista> clientes = query.getResultList();
	...
	public Optional<Transportista> findByNombreUsuario(String nombreUsuario) throws Exception {
		TypedQuery<Transportista> query = em.createQuery("SELECT t FROM Transportista t WHERE t.nombreUsuario = ?1", Transportista.class).setParameter(1, nombreUsuario);
		List<Transportista> clientes = query.getResultList();
		if (clientes != null && !clientes.isEmpty())
			return Optional.of(clientes.get(0));
